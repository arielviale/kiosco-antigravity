...
import { Package, ScanLine, ShoppingBag, TrendingUp } from 'lucide-react'
...
export default function Home() {
  ...
  const handleScanResult = (data: any | null, code: string) => {
    ...
    setScannedCode(code)
    if (!data) {
      setShowForm(false) // Reset form if new scan happens
    }
  }
      ...
      <div className="pt-28 px-4 max-w-md mx-auto space-y-10">
        {!producto && !showForm && (
          <div className="space-y-6 text-center animate-in fade-in duration-700">
            ...
            <div>
              <h2 className="text-3xl font-extrabold text-white mb-2">Escáner Listo</h2>
              <p className="text-slate-400">Escaneá un producto para empezar</p>
            </div>
